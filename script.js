...
function initRecentAndCharts(formRows, roleMap) {
    ...
    // Initialize custom dashboard views perfectly attached to existing load
...
// ===============================================
// NEW CUSTOM DASHBOARD FEATURES (Date Filter, KPIs, Top 10)
// ===============================================
...
function updateCustomNewDashboard() {
    ...
    rows.forEach(row => {
        ...
        if (inDateRange) {
            filteredCount++;
            if (['BH', 'CL', 'RCL', 'RBH'].includes(role)) {
            ...
            const region = row['Region (Audit)'] || row['Region (1-1)'] || 'Unknown';
            if (region && region !== 'Unknown') {
                regionMap[region] = (regionMap[region] || 0) + 1;
            }
        }

        // Stats tracking is calculated universally based on the overall data length, ignoring date filters
        if (!userStats[email]) {
            userStats[email] = { email: email, role: role, overall: 0, mtd: 0 };
        }
        userStats[email].overall++;
        if (isMTD) userStats[email].mtd++;
    });

    // 1. Update Custom KPIs
    ...
    if (ctxRegion) {
        ...
        customRegionChart = new Chart(ctxRegion, {
            ...
            data: {
                labels: regions,
                datasets: [{
                    label: 'Form Submissions (Filtered)',
                    data: regionCounts,
    ...
    // 4. Render Custom Top / Bottom 10 Tables
